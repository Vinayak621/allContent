...
def graph():
    if request.method == 'POST':
        ...
        for i in range(start_year, end_year + 1):
            ...
            spacialFiltered = temporalFiltered.filterBounds(point)
            ndvi_images = spacialFiltered.map(add_ndvi)
            circle = point.buffer(1000)
            imagecol = ndvi_images.qualityMosaic("NDVI")
            mean = imagecol.reduceRegion(
                reducer=ee.Reducer.mean(),
                geometry=circle,
                scale=30
            )
            val = mean.getInfo()['NDVI']
            ndvi_values.append(val)

        fig, ax = plt.subplots()
        ax.plot(range(start_year, end_year + 1), ndvi_values)
        ...
        buffer.seek(0)
        plot_str = base64.b64encode(buffer.read()).decode('utf-8')
        plot_data = f'data:image/png;base64,{plot_str}'

        return render_template('map_result.html', plot_data=plot_data,location_name=location_name)

    return render_template('ndvi_plot.html')
def get_location_name(latitude, longitude):
    api_key = '865db94453b58dea972413497a3aba29'
